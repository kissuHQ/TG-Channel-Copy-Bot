...
async function createTaskFromPage() {
  ...
  const result = await request('/api/tasks', {method:'POST', body:JSON.stringify({pair_ids:[pairId], mode, priority:document.getElementById('task-priority').value, limit:mode === 'last' ? value : 0, min_id:mode === 'from_id' ? value : 0})});
...
async function controlTask(id, action) {
  ...
  const result = action === 'cancel' ? await fetch(`/api/tasks/${encodeURIComponent(id)}`, {method:'DELETE'}).then(r=>r.json()) : await request(`/api/tasks/${encodeURIComponent(id)}`, {method:'PATCH', body:JSON.stringify(action === 'continue' ? {continue:true} : {paused:action === 'pause'})});
...
async function uploadPairThumbnail() { const pairId = document.getElementById('pair-id').value, file = document.getElementById('pair-thumbnail-file')?.files[0]; if (!pairId) return toast('Save the pair before uploading a thumbnail', true); if (!file) return toast('Choose a thumbnail image first', true); const body = new FormData(); body.append('thumbnail', file); const result = await fetch(`/api/pairs/${encodeURIComponent(pairId)}/thumbnail`, {method:'POST', body}).then(r => r.json()); toast(result.ok ? 'Thumbnail uploaded' : result.error, !result.ok); }
function pairPayload() { return {name:document.getElementById('pair-name').value.trim(), source:document.getElementById('pair-source').value.trim(), target:document.getElementById('pair-target').value.trim(), include_keywords:document.getElementById('pair-include').value, exclude_keywords:document.getElementById('pair-exclude').value, caption_prefix:document.getElementById('pair-prefix').value, caption_suffix:document.getElementById('pair-suffix').value, rate_delay:Number(document.getElementById('pair-rate').value || 3), max_messages:Number(document.getElementById('pair-max').value || 5000), daily_media_mb:Number(document.getElementById('pair-daily-mb').value || 2048), protected_behavior:document.getElementById('pair-protected').value, caption_enabled:document.getElementById('pair-caption-enabled').checked, thumbnail_enabled:document.getElementById('pair-thumbnail-enabled').checked, remove_links:document.getElementById('pair-links').checked, remove_source_name:document.getElementById('pair-source-name').checked, caption_template:document.getElementById('pair-caption-template').value, schedule_start:document.getElementById('pair-schedule-start').value, schedule_end:document.getElementById('pair-schedule-end').value, quiet_start:document.getElementById('pair-quiet-start').value, quiet_end:document.getElementById('pair-quiet-end').value, max_posts_per_hour:Number(document.getElementById('pair-hourly').value || 0), caption_parse_mode:document.getElementById('pair-caption-mode').value}; }
async function savePairFromPage() { const id = document.getElementById('pair-id').value, result = await request(id ? `/api/pairs/${encodeURIComponent(id)}` : '/api/pairs', {method:id ? 'PATCH' : 'POST', body:JSON.stringify(pairPayload())}); toast(result.ok ? 'Pair saved' : result.error, !result.ok); if (result.ok) { resetPairForm(); loadAppData(); } }
function editPairPage(id) { const pair = appData.pairs.find(item => item.id === id); if (!pair) return; document.getElementById('pair-id').value = id; document.getElementById('pair-form-title').textContent = 'Edit pair'; Object.entries({'pair-name':'name','pair-source':'source','pair-target':'target','pair-include':'include_keywords','pair-exclude':'exclude_keywords','pair-prefix':'caption_prefix','pair-suffix':'caption_suffix','pair-rate':'rate_delay','pair-max':'max_messages','pair-daily-mb':'daily_media_mb','pair-protected':'protected_behavior','pair-caption-template':'caption_template','pair-schedule-start':'schedule_start','pair-schedule-end':'schedule_end','pair-quiet-start':'quiet_start','pair-quiet-end':'quiet_end','pair-hourly':'max_posts_per_hour','pair-caption-mode':'caption_parse_mode'}).forEach(([id,key]) => { const el = document.getElementById(id); if (el) el.value = Array.isArray(pair[key]) ? pair[key].join(', ') : (pair[key] ?? ''); }); ['caption-enabled','thumbnail-enabled','links','source-name'].forEach(key => { const el = document.getElementById(`pair-${key}`); if (el) el.checked = !!pair[key === 'caption-enabled' ? 'caption_enabled' : key === 'thumbnail-enabled' ? 'thumbnail_enabled' : key === 'source-name' ? 'remove_source_name' : 'remove_links']; }); document.getElementById('pair-cancel').classList.remove('hidden'); location.hash = 'pair-editor'; }
function resetPairForm() { document.getElementById('pair-id').value = ''; document.getElementById('pair-form-title').textContent = 'Create a pair'; document.querySelectorAll('#pair-editor input:not([type=hidden]),#pair-editor textarea').forEach(el => { if (el.type === 'checkbox') el.checked = false; else el.value = ''; }); document.getElementById('pair-rate').value = 3; document.getElementById('pair-max').value = 5000; document.getElementById('pair-daily-mb').value = 2048; document.getElementById('pair-protected').value = 'download'; document.getElementById('pair-cancel').classList.add('hidden'); }
...
async function deletePairPage(id) { if (!confirm('Delete this channel pair?')) return; const result = await fetch(`/api/pairs/${encodeURIComponent(id)}`, {method:'DELETE'}).then(r => r.json()); toast(result.ok ? 'Pair deleted' : result.error, !result.ok); if (result.ok) loadAppData(); }
...
async function saveGlobalSettings() { const result = await request('/api/settings', {method:'PATCH', body:JSON.stringify({task_complete:document.getElementById('setting-complete').checked, task_failed:document.getElementById('setting-failed').checked, flood_wait:document.getElementById('setting-flood').checked, auto_forward:document.getElementById('setting-auto').checked})}); toast(result.ok ? 'Global settings saved' : result.error, !result.ok); }
async function setChannelPage(type) { const id = type === 'source' ? 'settings-src-input' : 'settings-tgt-input', value = document.getElementById(id).value.trim(); if (!value) return toast('Enter a channel first', true); const result = await request(`/api/set${type}`, {method:'POST', body:JSON.stringify({channel:value})}); toast(result.ok ? `${type} saved` : result.error, !result.ok); if (result.ok) loadAppData(); }
document.addEventListener('DOMContentLoaded', () => { loadAppData(); connectLive(); });